...
> **Note on NTU modules:** Scripts are available to scrape NTU module information (`get_ntu_module_info.py` and `get_ntu_module_descriptions.py`), but NTU data will **not** be included in our analysis due to time constraints. These scripts are retained for reference only.
...
> **Not used in analysis** — see note above.
...
Offline artifact builder for the chat app. Builds the retrieval index used by the HTTP retrieval API.
...
Starts the HTTP retrieval API that the Next.js chat UI calls.
