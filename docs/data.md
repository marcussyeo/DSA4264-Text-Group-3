...
1.  **Missing Description Handling**: Only 147 rows had missing descriptions and the `title` was used as a descriptive fallback. Project-like modules (e.g., "Dissertation", "UROPS") with missing descriptions were excluded from the primary text analysis to prevent noise.
2.  **Duplicate Check**: `moduleCode` is unique across all rows, confirming one row per module.
3.  **Feature Engineering**: A `level_band` feature was created by extracting the first digit of the `moduleCode` to distinguish undergraduate (1-4xxx) from postgraduate (5xxx+) courses.
...
1.  **Multi-label Field Preservation**: Multi-label fields (`skills`, `categories`, `position_levels`, `employment_types`) were preserved as Python lists to maintain their inherent structure for analysis.
2.  **Date Parsing**: All date fields were converted to datetime objects for temporal analysis.
...
![Category Co-occurrence](assets/EDA_job_seniority.png)
